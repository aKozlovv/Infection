...
final class SimulationController: UIViewController {
    ...
    /**
     Основной метод, который и производит симуляцию, а также отвечает за работу с потоками. 
     В глобальной очереди метод "пробегается" по индексам зараженных людей и для каждого из них вызывает метод для заражения его соседей.
     Как только количество инфецированных индексов становится равно количеству элементов в изначальном массиве  - метод останавливает симуляцию.
     */
    @objc
    ...
    /**
     функция вычисляет строку и столбец соответствующего индекса в массиве координат. Далее заполняется массив  соседних элементов (neighbor). 
     Это происходит путем проверки каждой координаты в массиве координат на то, чтобы она попадала в границы массива. 
     Если координата удовлетворяет этому условию и соответствует соседнему элементу в массиве, то  индекс элемента добавляется в массив neighbors.
     Перед возвратом массива neighbor метод также проверяет массив на соответствие infectionRate.
     */
    ...
    /**
     Методы, которые управляют таймером и запускают/останавливают процесс симуляции. 
     Таймер по заданному интервалу (infectionPeriod) вызывает основной метод симуляции - performSimulation
     */
